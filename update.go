// dotf update — 从 GitHub Releases 自更新。
//
...
// cmdUpdate 实现 dotf update 自更新。
//
//	usage:
//	  dotf update              更新到最新版本
//	  dotf update --check      仅检查是否有新版本（不下载/不替换）
//	  dotf update <version>    更新到指定版本（如 0.2.0 或 v0.2.0）
//	  dotf update --check <version>
func cmdUpdate(args []string) error {
	...
	for _, a := range args {
		...
		case strings.HasPrefix(a, "-"):
			return fmt.Errorf("update: unknown flag %q (see: dotf help)", a)
		default:
			if want != "" {
	...
	fmt.Printf("dotf: current %s, %s %s\n", version.Version, label, target)

	// 2. 比较版本
	switch cmp := compareSemver(target, version.Version); {
	case cmp == 0:
...
func replaceSelf(binData []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".dotf-update-*")
	if err != nil {
...
func permsErr(err error, path string) error {
	if os.IsPermission(err) {
		return fmt.Errorf("permission denied for %s: re-run with sudo, or install to a user directory:\n  make install (DOTFILES_PREFIX=$HOME/.local/bin)", path)
	}
	return err
}
